...
​	我并未使用任何IDE，采用Visual Studio Code代码编辑器 + clang + make在macOS上编译运行。本项目使用**纯C代码**编写，不调用任何std库函数，但使用了一些POSIX API. 经过测试在类Unix（macOS, Linux, BSD）系统上都能够编译运行。但由于MSVC不自带POSIX接口，在Windows下需要通过minGW环境进行编译。此外，众所周知只有Windows还没有全面使用UTF-8编码，这意味着在默认中文终端环境下（GBK编码），运行本程序将会乱码（因为在UTF-8环境下编写）。基于此，无论是源代码还是程序反馈我都使用英文编写以避免不必要的麻烦。
...
<img src="https://flexio.blob.core.windows.net/notch1p/2023/11/facf1d0e0cc1ad8fe8ba5ce5bb451c9b.png" alt="image-20231111002245746" style="zoom: 50%;" />
...
<img src="https://flexio.blob.core.windows.net/notch1p/2023/11/e455011be01324dafbaf20383a370c9c.png" alt="image-20231111002131097" style="zoom:50%;" />
...
![image-20231111002532002](https://flexio.blob.core.windows.net/notch1p/2023/11/e2af4bd0487f6aabde58058931786d9e.png)
...
> 编码"TWICE"并输出到stdout再重定向到test.bin

```bash
echo TWICE | ./main -c > test.bin
```
...
![image-20231111224437132](https://flexio.blob.core.windows.net/notch1p/2023/11/15d1a57627f980abd1f5183af691e633.png)

可以看出，解码后程序的md5和原文件相同，这证明了实现不存在重大错误。我们不妨将上述结果制成表格，如下所示：

（定义**压缩率**$r=\dfrac{S_{enc}}{S_{OG}}$（*越低越好*），其中$S$表示文件大小（in bytes）；$enc$表示编码文件；$OG$表示原文件；$n$表示符号数；$t_{enc}/t_{dec}$表示编码/解码时间（ms））
...
| $r\cdot 100\%$    | $56.764719\%$        | $51.831784\%$[^3]   | $25.967043\%$[^2] | $99.821909\%$ [^1]   |

因此，总有$t_{dec}\leq t_{enc},\,S_{enc}\leq S_{OG}.$ 这是符合预期和上面算法描述的。

[^1]: Word文件本身就是一个压缩包，故而压缩效果不明显
